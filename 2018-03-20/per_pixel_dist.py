...
outputs = [Div(text='<h1 style="text-align:center">TITLE HERE</h1>')]
...
f = figure(title='Region of Interest',
           x_range=[0, x_end-x_start], 
           y_range=[0, y_end-y_start],
           width=GRAPH_WIDTH)
f.image(image=[roi], x=0, y=0, dw=width, dh=height, palette="Greys256")
outputs.append(f)
# ==============================================================================
...
for slice_idx in range(inputs.shape[0]):
    ...
    if roi_coords:
        for abs_coords, rel_coords in roi_coords:
            ...
            data.append(PixelProb(*abs_coords, *probs))
